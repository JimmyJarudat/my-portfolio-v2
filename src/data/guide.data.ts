...
alias sstatus='sudo systemctl status'`,
    ...
    {
        id: 37, slug: "linux-essential-commands", category: "infrastructure",
        title: "Linux Essential Commands",
        description: "คำสั่ง Linux ที่ใช้บ่อยในงาน sysadmin ครอบคลุม file, process, user, network, disk และ archive — ใช้เป็น cheatsheet อ้างอิงได้ทันที",
        difficulty: "beginner", time: "15 min",
        tags: ["Linux", "Ubuntu", "Bash", "CLI"], updated: "May 2025",
        steps: [
            {
                title: "File & Directory",
                body: "คำสั่งจัดการไฟล์และโฟลเดอร์ที่ใช้บ่อยที่สุด",
                code: "ls -la                        # แสดงไฟล์ทั้งหมดพร้อม permission\nls -lh                        # แสดงขนาดแบบอ่านง่าย\npwd                           # แสดง path ปัจจุบัน\ncd /etc                       # เข้าโฟลเดอร์\ncd ~                          # กลับ home\ncd -                          # กลับโฟลเดอร์ก่อนหน้า\n\nmkdir -p /opt/app/logs        # สร้างโฟลเดอร์พร้อม parent\nrm -rf /tmp/old               # ลบโฟลเดอร์และทุกอย่างข้างใน\ncp -r /src /dst               # copy รวมโฟลเดอร์ย่อย\nmv /old/path /new/path        # ย้าย / เปลี่ยนชื่อ\nln -s /etc/nginx /nginx       # สร้าง symlink",
                lang: "bash",
                note: "ระวัง rm -rf — ไม่มี recycle bin บน Linux",
            },
            {
                title: "File Content & Search",
                body: "อ่าน ค้นหา และกรองเนื้อหาในไฟล์",
                code: "cat /etc/hosts                # แสดงทั้งไฟล์\nless /var/log/syslog          # อ่านแบบ scroll (q ออก)\nhead -20 file.log             # 20 บรรทัดแรก\ntail -f /var/log/nginx/access.log  # follow log realtime\n\ngrep -r \"error\" /var/log/     # ค้นหาในทุกไฟล์\ngrep -i \"failed\" auth.log     # case-insensitive\ngrep -n \"timeout\" app.log     # แสดงเลขบรรทัด\n\nfind /etc -name \"*.conf\"      # หาไฟล์ตามชื่อ\nfind /var -size +100M         # หาไฟล์ใหญ่กว่า 100MB\nfind /tmp -mtime +7 -delete   # ลบไฟล์เก่ากว่า 7 วัน\n\nwc -l file.txt                # นับบรรทัด",
                lang: "bash",
            },
            {
                title: "Process & System",
                body: "ดูและจัดการ process, service และข้อมูล system",
                code: "ps aux                        # แสดง process ทั้งหมด\nps aux | grep nginx           # กรอง process\ntop                           # monitor realtime (q ออก)\nhtop                          # top แบบ interactive\nkill 1234                     # ส่ง SIGTERM ให้ PID\nkill -9 1234                  # force kill\npkill nginx                   # kill ตามชื่อ\n\nsystemctl status nginx        # ดู status service\nsystemctl start|stop|restart nginx\nsystemctl enable nginx        # auto start เมื่อ boot\nsystemctl disable nginx\n\njournalctl -u nginx           # ดู log ของ service\njournalctl -u nginx --since today\njournalctl -f                 # follow log ทั้งหมด\n\nuptime                        # เวลา uptime + load avg\nfree -h                       # RAM ที่ใช้และว่าง\nuname -r                      # kernel version",
                lang: "bash",
            },
            {
                title: "User & Permission",
                body: "จัดการ user, group และสิทธิ์ไฟล์",
                code: "id                            # ดู uid, gid, groups ของตัวเอง\nwhoami                        # ชื่อ user ปัจจุบัน\n\nuseradd -m -s /bin/bash john  # สร้าง user พร้อม home\npasswd john                   # ตั้งรหัสผ่าน\nusermod -aG sudo john         # เพิ่มเข้า group\nuserdel -r john               # ลบ user พร้อม home\n\nsu - john                     # switch ไป user อื่น\nsudo command                  # รันด้วยสิทธิ์ root\n\n# Permission: r=4, w=2, x=1\nchmod 755 script.sh           # rwxr-xr-x\nchmod 600 private.key         # rw-------\nchmod -R 755 /var/www         # เปลี่ยนทั้งโฟลเดอร์\nchown john:john file.txt      # เปลี่ยน owner\nchown -R www-data /var/www",
                lang: "bash",
            },
            {
                title: "Network",
                body: "ตรวจสอบและจัดการ network interface, connection และ firewall",
                code: "ip addr show                  # ดู IP ทุก interface\nip addr show eth0             # ดูเฉพาะ interface\nip route show                 # ดู routing table\n\nping -c 4 8.8.8.8             # test connectivity\ncurl -I https://example.com   # ดู HTTP header\nwget -O /tmp/file https://... # download ไฟล์\n\nss -tlnp                      # ดู port ที่เปิดอยู่\nss -an | grep :80             # ดู connection บน port 80\n\nscp file.txt user@host:/path  # copy ไฟล์ผ่าน SSH\nssh -i key.pem user@host      # SSH ด้วย key\n\n# UFW Firewall\nsudo ufw status\nsudo ufw allow 443/tcp\nsudo ufw deny 23/tcp\nsudo ufw delete allow 80/tcp",
                lang: "bash",
            },
            {
                title: "Disk & Storage",
                body: "ตรวจสอบการใช้ disk และจัดการ mount",
                code: "df -h                         # disk usage ทุก partition\ndf -h /                       # เฉพาะ root partition\n\ndu -sh /var/log               # ขนาดรวมของโฟลเดอร์\ndu -sh /* | sort -hr          # เรียงตามขนาดใหญ่สุด\ndu -sh /var/log/* | sort -hr  # หาว่าอะไรกินพื้นที่\n\nlsblk                         # ดู disk และ partition ทั้งหมด\nblkid                         # ดู UUID ของ device\n\nmount /dev/sdb1 /mnt/data     # mount disk\numount /mnt/data              # umount\n\n# เพิ่มใน /etc/fstab เพื่อ auto mount\nUUID=xxxx /mnt/data ext4 defaults 0 2",
                lang: "bash",
            },
            {
                title: "Archive & Package",
                body: "บีบอัดไฟล์และจัดการ package บน Ubuntu/Debian",
                code: "# tar\ntar -czf backup.tar.gz /etc   # บีบอัดเป็น .tar.gz\ntar -xzf backup.tar.gz        # แตกไฟล์\ntar -tzf backup.tar.gz        # ดูเนื้อหาใน archive\n\n# zip\nzip -r archive.zip /path      # zip โฟลเดอร์\nunzip archive.zip -d /dst     # แตก zip\n\n# apt package manager\nsudo apt update               # อัปเดต package list\nsudo apt upgrade              # อัปเกรด package ทั้งหมด\nsudo apt install nginx        # ติดตั้ง package\nsudo apt remove nginx         # ลบ package\nsudo apt autoremove           # ลบ package ที่ไม่ใช้\napt search nginx              # ค้นหา package",
                lang: "bash",
            },
            {
                title: "Text Processing",
                body: "แปลง กรอง และจัดการข้อความ — ใช้บ่อยมากในงาน scripting และ log analysis",
                code: "# sed — แทนที่ข้อความ\nsed 's/old/new/g' file.txt             # แทนทุก occurrence\nsed -i 's/old/new/g' file.txt          # แก้ไฟล์ตรงๆ\nsed -n '10,20p' file.txt               # แสดงเฉพาะบรรทัด 10-20\nsed '/^#/d' file.conf                  # ลบบรรทัดที่ขึ้นต้นด้วย #\n\n# awk — ประมวลผลตาม column\nawk '{print $1, $3}' file.txt          # แสดง column 1 และ 3\nawk -F: '{print $1}' /etc/passwd       # ใช้ : เป็น delimiter\nawk '{sum += $1} END {print sum}' file # รวมค่าใน column 1\nawk '/error/ {print}' app.log          # กรองบรรทัดที่มี error\n\n# sort & uniq\nsort file.txt                          # เรียงตัวอักษร\nsort -n file.txt                       # เรียงตามตัวเลข\nsort -rn file.txt                      # เรียง reverse\nsort file.txt | uniq                   # ลบซ้ำ\nsort file.txt | uniq -c | sort -rn     # นับ + เรียงความถี่\n\n# cut & xargs\ncut -d: -f1 /etc/passwd                # ตัด field 1 ด้วย delimiter :\ncat list.txt | xargs rm                # ลบไฟล์ทุกชื่อในลิสต์\nfind /tmp -name '*.log' | xargs rm -f  # ใช้กับ find",
                lang: "bash",
            },
            {
                title: "Cron, Session & Shell",
                body: "ตั้ง scheduled job, จัดการ terminal session และ shell shortcut",
                code: "# crontab\ncrontab -l                    # ดู cron job ของ user ปัจจุบัน\ncrontab -e                    # แก้ไข cron job\nsudo crontab -l -u www-data   # ดู cron ของ user อื่น\n\n# format: นาที ชั่วโมง วัน เดือน วันในสัปดาห์\n# 0 2 * * *   /opt/backup.sh         # รันทุกวัน ตี 2\n# */5 * * * * /opt/check.sh          # รันทุก 5 นาที\n# 0 9 * * 1   /opt/report.sh         # รันทุกจันทร์ 9 โมง\n\n# tmux — session management\ntmux                          # สร้าง session ใหม่\ntmux new -s mywork            # สร้าง session ชื่อ mywork\ntmux ls                       # ดู session ที่มีอยู่\ntmux attach -t mywork         # กลับเข้า session\n# Ctrl+B, D → detach (session ยังรันอยู่)\n# Ctrl+B, C → new window\n# Ctrl+B, N/P → next/prev window\n\n# alias & shell\nalias ll='ls -la'             # สร้าง shortcut\nalias gs='git status'\nhistory                       # ดูคำสั่งที่ใช้ล่าสุด\nhistory | grep docker         # ค้นในประวัติ\n!123                          # รันคำสั่งหมายเลข 123 ซ้ำ\n\nenv                           # ดูทุก env variable\nexport PATH=$PATH:/opt/bin    # เพิ่ม path\necho $HOME $USER $SHELL       # ดู variable",
                lang: "bash",
                note: "เพิ่ม alias ใน ~/.bashrc หรือ ~/.zshrc เพื่อให้ใช้ได้ทุก session — แล้ว source ~/.bashrc เพื่อ reload",
            },
        ],
    },

    {
        id: 38, slug: "docker-essential-commands", category: "docker-swarm",
        title: "Docker Essential Commands",
        description: "คำสั่ง Docker ที่ใช้บ่อย ครอบคลุม container, image, compose, swarm stack, network, volume และ cleanup — ใช้เป็น cheatsheet อ้างอิงได้ทันที",
        difficulty: "beginner", time: "15 min",
        tags: ["Docker", "Docker Swarm", "Container", "CLI"], updated: "May 2025",
        steps: [
            {
                title: "Container Management",
                body: "สั่งรัน หยุด และจัดการ container",
                code: "docker ps                     # ดู container ที่รันอยู่\ndocker ps -a                  # ดูทั้งหมดรวม stopped\ndocker ps --format 'table {{.Names}}\\t{{.Status}}\\t{{.Ports}}'\n\ndocker run -d --name web -p 80:80 nginx   # รัน container\ndocker run -it ubuntu bash               # รันแบบ interactive\ndocker run --rm alpine echo hello        # รันแล้วลบทิ้ง\n\ndocker stop web               # หยุด container\ndocker start web              # รัน container ที่หยุดไว้\ndocker restart web            # restart\ndocker rm web                 # ลบ container (ต้อง stop ก่อน)\ndocker rm -f web              # force remove ทันที\n\ndocker exec -it web bash      # เข้า shell ใน container\ndocker exec web cat /etc/hosts\n\n# copy ไฟล์ระหว่าง host กับ container\ndocker cp web:/etc/nginx/nginx.conf ./nginx.conf   # container → host\ndocker cp ./nginx.conf web:/etc/nginx/nginx.conf   # host → container",
                lang: "bash",
            },
            {
                title: "Image Management",
                body: "ดึง สร้าง และจัดการ image",
                code: "docker images                 # ดู image ทั้งหมด\ndocker images -a              # รวม intermediate layers\n\ndocker pull nginx:latest      # ดึง image\ndocker pull nginx:1.25        # ระบุ version\n\ndocker build -t myapp:1.0 .   # build จาก Dockerfile\ndocker build -t myapp:1.0 -f Dockerfile.prod .\n\ndocker tag myapp:1.0 registry.io/myapp:1.0  # tag\ndocker push registry.io/myapp:1.0           # push ขึ้น registry\n\ndocker rmi nginx              # ลบ image\ndocker rmi -f nginx           # force remove\ndocker save myapp > myapp.tar # export image เป็นไฟล์\ndocker load < myapp.tar       # import image จากไฟล์",
                lang: "bash",
            },
            {
                title: "Logs & Inspect",
                body: "ดู log, resource usage และข้อมูลภายใน container",
                code: "docker logs web               # ดู log ทั้งหมด\ndocker logs -f web            # follow log realtime\ndocker logs --tail 100 web    # 100 บรรทัดล่าสุด\ndocker logs --since 1h web    # log ชั่วโมงที่แล้ว\n\ndocker inspect web            # ดูข้อมูลทั้งหมด JSON\ndocker inspect web | grep IPAddress  # กรอง IP\n\ndocker stats                  # monitor resource realtime\ndocker stats --no-stream      # snapshot ครั้งเดียว\n\ndocker top web                # ดู process ใน container\ndocker diff web               # ดูไฟล์ที่เปลี่ยนจาก image",
                lang: "bash",
            },
            {
                title: "Network & Volume",
                body: "จัดการ network และ volume สำหรับ container",
                code: "# Network\ndocker network ls             # ดู network ทั้งหมด\ndocker network create mynet   # สร้าง bridge network\ndocker network create --driver overlay main_public  # overlay สำหรับ swarm\ndocker network inspect mynet  # ดูข้อมูล network\ndocker network connect mynet web        # เชื่อม container กับ network\ndocker network disconnect mynet web\n\n# Volume\ndocker volume ls              # ดู volume ทั้งหมด\ndocker volume create mydata   # สร้าง volume\ndocker volume inspect mydata  # ดูข้อมูล volume\ndocker volume rm mydata       # ลบ volume\n\n# ใช้ volume ตอน run\ndocker run -v mydata:/app/data nginx\ndocker run -v /host/path:/container/path nginx  # bind mount\n\n# Secret (Swarm เท่านั้น)\necho \"mypassword\" | docker secret create db_pass -   # สร้าง secret จาก stdin\ndocker secret ls              # ดู secret ทั้งหมด\ndocker secret inspect db_pass # ดูข้อมูล (ไม่แสดง value)\ndocker secret rm db_pass      # ลบ secret",
                lang: "bash",
            },
            {
                title: "Docker Compose",
                body: "จัดการ multi-container application ด้วย compose",
                code: "docker compose up -d          # รัน services ทั้งหมด\ndocker compose up -d --build  # build ใหม่ก่อนรัน\ndocker compose down           # หยุดและลบ container\ndocker compose down -v        # ลบ volume ด้วย\n\ndocker compose ps             # ดู status\ndocker compose logs -f        # follow log ทุก service\ndocker compose logs -f web    # log เฉพาะ service\n\ndocker compose exec web bash  # เข้า shell\ndocker compose restart web    # restart service\ndocker compose pull           # ดึง image ใหม่\ndocker compose config         # validate และดู config",
                lang: "bash",
            },
            {
                title: "Docker Swarm & Stack",
                body: "Deploy และจัดการ service บน Docker Swarm",
                code: "# Swarm init\ndocker swarm init --advertise-addr 192.168.1.10  # init manager\ndocker swarm join --token ... 192.168.1.10:2377  # เพิ่ม worker\ndocker node ls                # ดู node ทั้งหมดใน swarm\n\n# Stack deploy\ndocker stack deploy -c stack.yml myapp  # deploy stack\ndocker stack ls               # ดู stack ทั้งหมด\ndocker stack rm myapp         # ลบ stack\ndocker stack services myapp   # ดู services ใน stack\n\n# Service\ndocker service ls             # ดู service ทั้งหมด\ndocker service ps myapp_web   # ดู task ของ service\ndocker service logs -f myapp_web  # log ของ service\ndocker service update --image nginx:1.25 myapp_web  # update image\ndocker service scale myapp_web=3  # scale replicas",
                lang: "bash",
            },
            {
                title: "Cleanup",
                body: "ทำความสะอาด resource ที่ไม่ใช้แล้ว",
                code: "docker system df              # ดูพื้นที่ที่ Docker ใช้\ndocker system prune           # ลบ container, network, image ที่ไม่ใช้\ndocker system prune -a        # ลบทุกอย่างที่ไม่มี container ใช้\ndocker system prune -a --volumes  # รวม volume ด้วย\n\ndocker container prune        # ลบเฉพาะ stopped container\ndocker image prune            # ลบ dangling image\ndocker image prune -a         # ลบ image ที่ไม่มี container ใช้\ndocker volume prune           # ลบ volume ที่ไม่ใช้\ndocker network prune          # ลบ network ที่ไม่ใช้",
                lang: "bash",
                note: "docker system prune -a --volumes จะลบ volume ด้วย ระวังข้อมูลหาย — ใช้เฉพาะบน dev environment",
            },
        ],
    },

    {
        id: 39, slug: "windows-server-commands", category: "infrastructure",
        title: "Windows Server Commands",
        description: "คำสั่ง PowerShell และ CMD สำหรับ Windows Server ที่ใช้บ่อย ครอบคลุม service, network, user, disk, event log และ remote management",
        difficulty: "beginner", time: "15 min",
        tags: ["Windows Server", "PowerShell", "CMD", "CLI"], updated: "May 2025",
        steps: [
            {
                title: "Service & Process",
                body: "จัดการ service และ process บน Windows Server",
                code: "# Service\nGet-Service                          # ดู service ทั้งหมด\nGet-Service -Name nginx              # ดูเฉพาะ service\nGet-Service | Where-Object {$_.Status -eq 'Running'}\nStart-Service nginx\nStop-Service nginx\nRestart-Service nginx\nSet-Service nginx -StartupType Automatic  # ตั้ง auto start\n\n# Process\nGet-Process                          # ดู process ทั้งหมด\nGet-Process -Name chrome             # กรองตามชื่อ\nStop-Process -Name chrome -Force     # ปิด process\nStop-Process -Id 1234 -Force\n\n# CMD แบบเดิม\nsc query nginx\nsc start nginx\nsc stop nginx\ntasklist\ntaskkill /PID 1234 /F",
                lang: "powershell",
            },
            {
                title: "Network",
                body: "ตรวจสอบ network, port และ connectivity",
                code: "# IP & Interface\nipconfig /all                        # ดู IP ทุก interface\nipconfig /flushdns                   # ล้าง DNS cache\nipconfig /release && ipconfig /renew # ต่ออายุ DHCP\n\n# Connectivity\nping 8.8.8.8\nping -t 8.8.8.8                      # ping ต่อเนื่อง\ntracert google.com\npathping google.com                  # tracert + packet loss\n\n# DNS\nnslookup google.com\nnslookup google.com 8.8.8.8          # ใช้ DNS server กำหนดเอง\nResolve-DnsName google.com\n\n# Port & Connection\nnetstat -ano                         # ดู connection ทั้งหมด\nnetstat -ano | findstr :80           # กรองตาม port\nTest-NetConnection -ComputerName server1 -Port 443\nTest-NetConnection server1 -Port 3389 -InformationLevel Detailed",
                lang: "powershell",
            },
            {
                title: "User & Active Directory",
                body: "จัดการ user บน local machine และ Active Directory",
                code: "# Local User\nnet user                             # ดู user ทั้งหมด\nnet user john Password1! /add        # สร้าง user\nnet user john /delete                # ลบ user\nnet localgroup Administrators john /add\n\n# Active Directory (ต้องมี RSAT)\nGet-ADUser -Identity john            # ดูข้อมูล user\nGet-ADUser -Filter * | Select Name, Enabled\nGet-ADUser john | Select *           # ดูทุก property\n\nNew-ADUser -Name \"John\" -SamAccountName john -AccountPassword (ConvertTo-SecureString \"Pass1!\" -AsPlainText -Force) -Enabled $true\nDisable-ADAccount -Identity john\nEnable-ADAccount -Identity john\nSet-ADAccountPassword -Identity john -Reset -NewPassword (ConvertTo-SecureString \"NewPass1!\" -AsPlainText -Force)\n\nGet-ADComputer -Filter * | Select Name, OperatingSystem\nGet-ADGroupMember -Identity \"Domain Admins\"",
                lang: "powershell",
            },
            {
                title: "Disk & Storage",
                body: "ตรวจสอบและจัดการ disk, partition และพื้นที่เก็บข้อมูล",
                code: "# Disk Info\nGet-PSDrive -PSProvider FileSystem  # ดู drive ทั้งหมดพร้อม free space\nGet-Volume                           # ดู volume ทั้งหมด\nGet-Disk                             # ดู physical disk\nGet-Partition                        # ดู partition ทั้งหมด\n\n# หาโฟลเดอร์ที่กินพื้นที่มาก\nGet-ChildItem C:\\ -Recurse -ErrorAction SilentlyContinue |\nSort-Object Length -Descending | Select-Object -First 20 FullName, Length\n\n# Disk Cleanup\nCleanmgr /sagerun:1                  # Disk Cleanup อัตโนมัติ\n\n# diskpart (CMD)\n# diskpart\n# list disk\n# select disk 1\n# list partition",
                lang: "powershell",
                note: "Get-ChildItem แบบ Recurse บน C:\\ ใช้เวลานาน รอสักครู่",
            },
            {
                title: "Event Log",
                body: "ค้นหาและอ่าน Event Log สำหรับ troubleshooting",
                code: "# ดู Event Log\nGet-EventLog -LogName System -Newest 50\nGet-EventLog -LogName Application -Newest 50 -EntryType Error\nGet-EventLog -LogName Security -Newest 100 | Where-Object {$_.EventID -eq 4624}  # Logon success\nGet-EventLog -LogName Security | Where-Object {$_.EventID -eq 4625}  # Logon failure\n\n# Get-WinEvent (ใหม่กว่า รองรับ filter ดีกว่า)\nGet-WinEvent -LogName System -MaxEvents 50\nGet-WinEvent -FilterHashtable @{LogName='System'; Level=2; StartTime=(Get-Date).AddDays(-1)}\nGet-WinEvent -FilterHashtable @{LogName='Security'; Id=4625}  # Failed login\n\n# Export log\nGet-WinEvent -LogName System -MaxEvents 1000 | Export-Csv C:\\logs\\system.csv -NoTypeInformation\n\n# Event ID ที่ต้องรู้:\n# 4624 - Logon success\n# 4625 - Logon failure\n# 4634 - Logoff\n# 4740 - Account locked out\n# 4720 - User created\n# 6005/6006 - System start/shutdown",
                lang: "powershell",
            },
            {
                title: "System Info & Remote",
                body: "ดูข้อมูล system และจัดการ remote connection",
                code: "# System Info\nsysteminfo                           # ข้อมูล system ทั้งหมด\nGet-ComputerInfo | Select OsName, OsVersion, CsProcessors\n[System.Environment]::OSVersion      # OS version\n$env:COMPUTERNAME                    # ชื่อเครื่อง\n\n# Uptime\n(Get-Date) - (gcim Win32_OperatingSystem).LastBootUpTime\n\n# Remote PowerShell\nEnter-PSSession -ComputerName server1 -Credential domain\\admin\nInvoke-Command -ComputerName server1 -ScriptBlock { Get-Service }\nInvoke-Command -ComputerName server1,server2 -ScriptBlock { ipconfig }\n\n# RDP\nmstsc /v:server1                     # เปิด RDP ไปยัง server\n\n# WinRM (ต้อง enable)\nEnable-PSRemoting -Force\nSet-Item WSMan:\\localhost\\Client\\TrustedHosts -Value \"server1,server2\"",
                lang: "powershell",
            },
            {
                title: "Firewall, Scheduled Task & GPO",
                body: "จัดการ firewall rules, scheduled task และ Group Policy — ใช้บ่อยมากในงาน Windows Server admin",
                code: "# Windows Firewall\nGet-NetFirewallRule | Where-Object {$_.Enabled -eq 'True'} | Select DisplayName, Direction\nNew-NetFirewallRule -DisplayName 'Allow SQL' -Direction Inbound -Protocol TCP -LocalPort 1433 -Action Allow\nDisable-NetFirewallRule -DisplayName 'Allow SQL'\nRemove-NetFirewallRule -DisplayName 'Allow SQL'\n\n# Scheduled Task\nGet-ScheduledTask                    # ดู task ทั้งหมด\nGet-ScheduledTask -TaskName 'Backup' # ดู task ชื่อ Backup\nStart-ScheduledTask -TaskName 'Backup'  # รัน task ทันที\nDisable-ScheduledTask -TaskName 'Backup'\nEnable-ScheduledTask -TaskName 'Backup'\n\n# สร้าง Scheduled Task ด้วย PowerShell\n$action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument '-File C:\\scripts\\backup.ps1'\n$trigger = New-ScheduledTaskTrigger -Daily -At '02:00'\nRegister-ScheduledTask -TaskName 'NightlyBackup' -Action $action -Trigger $trigger -RunLevel Highest\n\n# Group Policy\ngpupdate /force                      # บังคับ apply GPO ทันที\ngpresult /r                          # ดู GPO ที่ apply กับ user/computer\ngpresult /h C:\\gpo-report.html       # export รายงาน HTML\n\n# System Repair\nsfc /scannow                         # ตรวจและซ่อม system file\nDISM /Online /Cleanup-Image /RestoreHealth  # ซ่อม Windows image",
                lang: "powershell",
            },
        ],
    },
    ...
    {
        id: 40, slug: "network-troubleshooting-commands", category: "observability",
        title: "Network Troubleshooting Commands",
        description: "คำสั่ง troubleshoot network บน Linux, Windows และ MikroTik ครอบคลุม connectivity, DNS, port scanning, packet capture และ HTTP testing",
        difficulty: "beginner", time: "15 min",
        tags: ["Network", "Linux", "Windows", "MikroTik", "Wireshark"], updated: "May 2025",
        steps: [
            {
                title: "Connectivity Test",
                body: "ทดสอบการเชื่อมต่อเบื้องต้นและ trace เส้นทาง packet",
                code: "# ping\nping 8.8.8.8                  # test ว่าเชื่อมต่อได้\nping -c 4 8.8.8.8             # ping 4 ครั้ง (Linux)\nping -t 8.8.8.8               # ping ต่อเนื่อง (Windows)\nping -s 1400 8.8.8.8          # กำหนด packet size (Linux)\n\n# traceroute / tracert\ntraceroute 8.8.8.8            # Linux\ntracert 8.8.8.8               # Windows\ntraceroute -T -p 443 8.8.8.8  # ใช้ TCP แทน ICMP\n\n# mtr (Linux — รวม ping + traceroute)\nmtr 8.8.8.8\nmtr --report 8.8.8.8          # สรุปผล 10 rounds\nmtr -n 8.8.8.8                # ไม่ resolve hostname\n\n# pathping (Windows)\npathping google.com           # ดู latency + packet loss ทุก hop",
                lang: "bash",
            },
            {
                title: "DNS Lookup",
                body: "ตรวจสอบ DNS record และ resolve hostname",
                code: "# nslookup (Linux + Windows)\nnslookup google.com           # query DNS default\nnslookup google.com 8.8.8.8   # ระบุ DNS server\nnslookup -type=MX google.com  # ดู MX record\nnslookup -type=TXT google.com # ดู TXT record\n\n# dig (Linux)\ndig google.com                # query A record\ndig google.com MX             # MX record\ndig google.com ANY            # ทุก record\ndig @8.8.8.8 google.com       # ระบุ DNS server\ndig +short google.com         # แสดงแค่ IP\ndig -x 8.8.8.8                # reverse lookup\n\n# host (Linux)\nhost google.com\nhost -t MX google.com\n\n# Flush DNS cache\nipconfig /flushdns            # Windows\nresolvectl flush-caches       # Ubuntu 22.04+\nsudo systemd-resolve --flush-caches",
                lang: "bash",
            },
            {
                title: "Port & Socket",
                body: "ตรวจสอบ port ที่เปิดอยู่และ connection",
                code: "# ss (Linux — แทน netstat)\nss -tlnp                      # TCP listening ports พร้อม process\nss -ulnp                      # UDP listening ports\nss -an | grep :80             # connection บน port 80\nss -tn state established      # connection ที่ established\n\n# netstat\nnetstat -tlnp                 # Linux\nnetstat -ano                  # Windows\nnetstat -ano | findstr :443   # Windows — filter port\n\n# nc (netcat) — test port connectivity\nnc -zv 192.168.1.1 22         # test ว่า port เปิดไหม\nnc -zv 192.168.1.1 80 443     # test หลาย port\nnc -zvw 3 host 3306           # timeout 3 วินาที\n\n# Test-NetConnection (Windows)\nTest-NetConnection 192.168.1.1 -Port 3306\nTest-NetConnection google.com -Port 443\n\n# telnet (ถ้า enable)\ntelnet 192.168.1.1 22",
                lang: "bash",
            },
            {
                title: "Interface & Route",
                body: "ดูและจัดการ network interface และ routing table",
                code: "# Linux\nip addr show                  # ดู IP ทุก interface\nip addr show eth0             # เฉพาะ interface\nip link show                  # ดู interface state\nip link set eth0 up/down      # เปิด/ปิด interface\n\nip route show                 # ดู routing table\nip route add 10.0.0.0/8 via 192.168.1.1  # เพิ่ม route\nip route del 10.0.0.0/8       # ลบ route\nip route get 8.8.8.8          # ดูว่าจะไปทาง interface ไหน\n\narp -n                        # ดู ARP table (Linux)\narp -a                        # Windows/Linux\nip neigh show                 # ARP table แบบใหม่ (Linux)\n\n# Windows\nipconfig /all\nroute print                   # ดู routing table\nroute add 10.0.0.0 mask 255.0.0.0 192.168.1.1  # เพิ่ม route\narp -a                        # ARP table",
                lang: "bash",
            },
            {
                title: "Packet Capture",
                body: "จับ packet เพื่อวิเคราะห์ traffic บน network",
                code: "# tcpdump (Linux)\ntcpdump -i eth0               # จับ packet บน interface\ntcpdump -i eth0 -n            # ไม่ resolve hostname\ntcpdump -i eth0 port 80       # กรองเฉพาะ port 80\ntcpdump -i eth0 host 8.8.8.8  # กรองตาม host\ntcpdump -i eth0 -w capture.pcap  # บันทึกเป็นไฟล์\ntcpdump -i eth0 -c 100        # จับแค่ 100 packet\n\n# กรองหลายเงื่อนไข\ntcpdump -i eth0 'port 80 or port 443'\ntcpdump -i eth0 'src 192.168.1.10 and port 443'\ntcpdump -i eth0 'tcp[tcpflags] & tcp-syn != 0'  # เฉพาะ SYN\n\n# อ่านไฟล์ pcap\ntcpdump -r capture.pcap\ntcpdump -r capture.pcap -n port 443\n\n# tshark (command line Wireshark)\ntshark -i eth0\ntshark -i eth0 -f 'port 80'\ntshark -r capture.pcap",
                lang: "bash",
                note: "ต้องใช้ sudo สำหรับ tcpdump — ไฟล์ .pcap เปิดได้ด้วย Wireshark บนเครื่อง local",
            },
            {
                title: "HTTP Testing",
                body: "ทดสอบ HTTP/HTTPS endpoint, header และ response",
                code: "# curl\ncurl https://example.com                    # GET request\ncurl -I https://example.com                 # ดูแค่ header\ncurl -v https://example.com                 # verbose ดูทุกอย่าง\ncurl -o /dev/null -s -w '%{http_code}' https://example.com  # แค่ status code\n\ncurl -X POST https://api.example.com/login \\\n  -H 'Content-Type: application/json' \\\n  -d '{\"user\":\"admin\",\"pass\":\"secret\"}'\n\ncurl -H 'Authorization: Bearer TOKEN' https://api.example.com/data\ncurl -k https://self-signed.example.com     # ข้าม SSL verify\ncurl -L https://example.com                 # ตาม redirect\ncurl --max-time 10 https://example.com      # timeout 10 วินาที\n\n# wget\nwget https://example.com                    # download\nwget -q -O- https://example.com | head     # อ่าน output\nwget --spider https://example.com           # ตรวจว่า URL มีอยู่",
                lang: "bash",
            },
            {
                title: "Bandwidth & SSL Test",
                body: "ทดสอบ bandwidth จริง และตรวจสอบ SSL/TLS certificate",
                code: "# iperf3 — ทดสอบ bandwidth ระหว่างสอง host\n# บน server (ฝั่งรับ)\niperf3 -s\niperf3 -s -p 5202              # ระบุ port\n\n# บน client (ฝั่งส่ง)\niperf3 -c 192.168.1.10         # TCP test\niperf3 -c 192.168.1.10 -t 30  # test 30 วินาที\niperf3 -c 192.168.1.10 -u -b 100M  # UDP test 100Mbps\niperf3 -c 192.168.1.10 -R      # reverse (วัดขาลง)\n\n# openssl — ตรวจสอบ SSL/TLS\nopenssl s_client -connect example.com:443  # ดู certificate chain\nopenssl s_client -connect example.com:443 </dev/null 2>/dev/null | openssl x509 -noout -dates  # วันหมดอายุ\nopenssl s_client -connect example.com:443 -servername example.com  # SNI\ncurl -vI https://example.com 2>&1 | grep -A5 'SSL'  # ดู SSL ผ่าน curl\n\n# iftop — monitor bandwidth realtime per connection (Linux)\nsudo apt install iftop\nsudo iftop -i eth0             # monitor interface eth0\nsudo iftop -i eth0 -n          # ไม่ resolve hostname\n\n# nethogs — bandwidth per process (Linux)\nsudo apt install nethogs\nsudo nethogs eth0              # ดูว่า process ไหนกิน bandwidth",
                lang: "bash",
                note: "iperf3 ต้องติดตั้งทั้งสองฝั่ง — sudo apt install iperf3 (Linux) หรือ ดาวน์โหลดได้จาก iperf.fr (Windows)",
            },
            {
                title: "MikroTik Tools",
                body: "คำสั่ง troubleshoot network บน MikroTik RouterOS ผ่าน Terminal",
                code: "# Ping & Traceroute\n/tool ping 8.8.8.8 count=4\n/tool ping 8.8.8.8 interface=ether1\n/tool traceroute 8.8.8.8\n\n# Bandwidth Test (ระหว่าง 2 router MikroTik)\n/tool bandwidth-test address=192.168.1.2 direction=both\n\n# Torch — monitor traffic realtime บน interface\n/tool torch interface=ether1\n/tool torch interface=ether1 ip-protocol=tcp\n\n# Packet Sniffer\n/tool sniffer start\n/tool sniffer stop\n/tool sniffer print\n/tool sniffer save file-name=capture\n\n# DNS\n/ip dns cache print            # ดู DNS cache\n/ip dns cache flush            # ล้าง DNS cache\n/tool dns-lookup google.com\n\n# ARP & Route\n/ip arp print                  # ดู ARP table\n/ip route print                # ดู routing table\n\n# Firewall Connection\n/ip firewall connection print  # ดู active connection",
                lang: "bash",
                note: "MikroTik commands ใช้ผ่าน Winbox Terminal, SSH หรือ WebFig Terminal",
            },
        ],
    },
